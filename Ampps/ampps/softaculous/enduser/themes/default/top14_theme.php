...
if(!empty($updates_available)){	
	...
	$i=1;
		...
		foreach($top as $k => $v){
			...
			$str .= '<td width="'.$td_width.'" valign="middle" align="center" height="'.$td_height.'" style="'.$td_pading.'"><a href="'.$hrefurl.'" style="text-decoration:none;'.$softac_cat_color.'"><img src="'.$imgsrc.'" border="0" id="top14scriptbg"/><br />'.$link_name.'</a></td>';
			$i++;
...
$str .= '<div class="softac_cat" style="vertical-align:middle;padding:5px;text-align:left;background-color: '.$softac_cat_bgcolor.';width:'.$width.';'.$softac_cat_color.'"><b>'.$l['category'].' :</b></div>
		...
		foreach($allcatwise as $type => $cats){
			foreach($cats as $k => $v){
				...
				$str .= ' <td width="'.$td_width.'" valign="top" align="center"><a href="'.$fullurl.'/'.$globals['index'].'ind='.$type.'&act=listsoftwares&cat='.$k.'" style="text-decoration:none;'.$softac_cat_color.'" class="desc">'.((is_cp_theme('paper_lantern') || is_cp_theme('jupiter') || is_cp_theme('x3') || $globals['softpanel'] == 'webuzo') ? '<div class="top14catbg"><i class="fas sai-'.$k.(is_cp_theme('x3') ? ' fa-1x' : ' fa-2x').'"></i></div>' : '<img src="'.$fullurl.'/'.$cat_image_path.$type.'_'.$k.'.gif" title="'.strtoupper($type).' '.strtoupper($k).'" border="0">').(!is_cp_theme('x3') ? '<br />' : '').'<div id="top14catmargin" style="'.$softac_cat_color.'">'.$l['cat_'.$type.'_'.$k].'</div></a></td>';
