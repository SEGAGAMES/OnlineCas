...
function restore_template_theme(){
	...
	$return .= '<a id="sclone_overview" '.$role.' href="'.script_link($soft).'&highlight='.$new_insid.'&postact=restore_template"><span class="btn btn-outline-primary">'.$l['return'].'</span></a>';
	...
	echo '
	<div class="bg" id="completed" '.(empty($restored) ? 'style="display:none;"' : '').'>
		<h5>'.$l['congrats'].'</h5>
		...
		<div class="setupcontinue">'.(!empty($setupcontinue) || empty($restored) ? $l['setup_continue'].' : <br />
		...
		if(empty($globals['restore_template_tweet_off']) || empty($globals['lictype'])){
			echo '
			<form method="get" action="https://twitter.com/intent/tweet" id="tweet" onsubmit="return dotweet(this);">
				...
				<div class="card">
					<div class="card-header" style="font-size:14px;">
						<font><b>'.$l['restore_template_tweet_sub'].' :</b></font>
					</div>
					<div class="card-body">
						<div class="row">
							<div class="col-sm-12">
								<textarea id="txttweet" name="text" cols="80" row="3" style="resize:none; width:100%;">'.loadtweetdata('restore_template_tweet', $l['restore_template_tweet']).'</textarea>
							</div>
						</div>
						<br />
						<div class="row">
					...
					<div class="bg"><br />						
						<div class="row sai_main_head mt-8 mb-4">
							<div class="col-12 text-center">
								<i class="fas fa-window-restore fa-2x mr-2" style="color:#00A0D2;"></i>
								<span>'.$l['restore_template'].'</span>
							</div>
						</div>';
								...
								<div class="col-sm-6" style="padding-left:0px; padding-right:0px;">
									<select name="softdomain" class="form-control" id="softdomain" onchange="softmail();change_admin_prefix(this.value);" onblur="checkhttps(\'softproto\', \'softdomain\', false)">';
										foreach($softpanel->domainroots as $domain => $dompath){
											echo '<option value="'.$domain.'" '.POSTselect('softdomain', $domain, ($source_ins['softdomain'] == $domain)).'>'.$domain.'</option>';
										}
										echo'
									</select>
						...
						<div class="bg m-3 info_block">
							...
							<div class="row mb-3">
								<div class="col-5">
									<span class="sai_head">'.$l['template_name'].'</span>
								</div>
								<div class="col-6">
									<span class="sai_info">'.base64_decode($source_ins['template_name']).'</span>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-6">
									<span class="sai_info">'.$source_ins['ver'].'</span>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-6">
									<span class="sai_info">'.datify($source_ins['btime']).'</span>
								</div>
							</div>
							...
							<div class="row mb-3">
								...
								<div class="col-6">
									<span class="sai_info"><a href="'.$source_ins['softurl'].'" target="_blank">'.$source_ins['softurl'].'</a></span>
								</div>
							</div>
				...
				<div class="bg">
					<center>
						<div class="row sai_main_head" style="width:100%;" align="center">
							...
							<div class="col-sm-7 col-xs-7" style="padding-top:10px; padding-left:0; text-align:left;">'.$l['prog_cloning'].'</div>
						</div><hr /><br />
