...
// $data = source installation details
// $settings = destination details
// $backupinfo = backupinfo details
...
// empty($data['backup_dir']):this condition is empty because while unzipping DIR we are going to extract everything anyways
if(empty($data['backup_dir']) && !empty($data['backup_datadir']) && !empty($data['backup_wwwdir'])){
	untar_archive($settings['softpath'].'/'.$data['fname'], $settings['softpath'], array('softperms.txt', 'softver.txt'), true);
}

// Restore files
if(!empty($data['backup_dir']) && empty($GLOBALS['current_status'])){
	...
	if(!empty($data['soft_version'])){
		...
		if(!untar_archive($settings['softpath'].'/'.$data['fname'], $settings['softpath'], array(), true)){
	...
	$perms = @file($settings['softpath'].'/softperms.txt');
	if(is_array($perms)){
		foreach($perms as $k => $v){
			...
			if(is_numeric($perm)){
				...
				if(empty($link)){
					$dest_file = $settings['softpath'].'/'.substr($v, 0, -5);
				}else{
					$dest_file = $link;
				}
...
// Restore Database	
if(!empty($data['backup_db']) && $GLOBALS['current_status'] < 2){
	...
	// Does the USER exist ?
	$mysql = @soft_mysql_connect($settings['softdbhost'], $dbuser, $dbpass, true);
	
	// Try to select the DB if connection was successful
	if($mysql){
		$_mysql = @soft_mysql_select_db($settings['softdb'], $mysql);
	}
	
	// Could not connect to the DB, so try to CREATE
	if(!empty($data['create_db']) && empty($_mysql)){
		if(!$softpanel->createdb($settings['softdb'], $dbuser, $dbpass)){
	...
	// Only for TAR
	if(!empty($data['soft_version'])){
		if(!file_exists($settings['softpath'].'/'.$data['dbexist'])){
			if(!untar_archive($settings['softpath'].'/'.$data['fname'], $settings['softpath'], array($data['dbexist']), true)){
				$error[] = 'There was some error while unzipping the backup files';
				softdie('restoreerror');
			}
		}
	...
	//$sql_data = file_get_contents($settings['softpath'].'/'.$data['dbexist']);
	
	//Make the Connection
	$__conn = @soft_mysql_connect($settings['softdbhost'], $dbuser, $dbpass, true);
...
'.soft_mysql_error($__conn);
	...
	soft_import($settings['softpath'].'/'.$data['dbexist'], $__conn);
	
	if(!empty($error)){
		return false;
	}
	
	@unlink($settings['softpath'].'/'.$data['dbexist']);
	if(file_exists($settings['softpath'].'/'.$data['fname'].'.partial')){
		@unlink($settings['softpath'].'/'.$data['fname'].'.partial');
	}
	//r_print($data);
...
if(!empty($data['backup_datadir']) && $GLOBALS['current_status'] < 3){
	...
	if(!file_exists($settings['softpath'].'/softdatadir')){
		if(!untar_archive($settings['softpath'].'/'.$data['fname'], $settings['softpath'], array('softdatadir/*'), true)){
	...
	if(!copydir_fn($settings['softpath'].'/softdatadir', $settings['softdatadir'])){
		$error[] = 'Unable to restore data directory';
		softdie('restoredatadir');
	}
	
	rmdir_recursive_fn($settings['softpath'].'/softdatadir');
	
	// See if a permission list is there ?
	$perms = @file($settings['softdatadir'].'/softperms.txt');
	if(is_array($perms)){
		foreach($perms as $k => $v){
			$v = trim($v);
			$perm = substr($v, -4);
			if(is_numeric($perm)){
				@chmod($settings['softdatadir'].'/'.substr($v, 0, -5), octdec($perm));
			}
		}
	}
...
//Restore the cron
/* if(!empty($settings['cron_time']) && !empty($settings['cron_command'])){

	$cron_time = explode(',', $settings['cron_time']);
	$softpanel->addcron($cron_time[0], $cron_time[1], $cron_time[2], $cron_time[3], $cron_time[4], base64_decode($settings['cron_command']));
	
} */


//Restore Web directory
if(!empty($data['backup_wwwdir']) && $GLOBALS['current_status'] < 4){
	...
	if(!file_exists($settings['softpath'].'/wwwdir')){
		if(!untar_archive($settings['softpath'].'/'.$data['fname'], $settings['softpath'], array('wwwdir/*'), true)){
	...
	if(!copydir_fn($settings['softpath'].'/wwwdir', $settings['wwwdir'])){
		$error[] = 'Unable to restore Web directory';
		softdie('restorewww');
	}
	
	rmdir_recursive_fn($settings['softpath'].'/wwwdir');
	
	// See if a permission list is there ?
	$perms = @file($settings['wwwdir'].'/softperms.txt');
	if(is_array($perms)){
		foreach($perms as $k => $v){
			$v = trim($v);
			$perm = substr($v, -4);
			if(is_numeric($perm)){
				@chmod($settings['wwwdir'].'/'.substr($v, 0, -5), octdec($perm));
			}
		}
	}
