...
if(empty($error)){
	...
	if(!empty($__settings['softdatadir'])){
		...
		$source_datadir = '/'.basename($__settings['source_data']['softdatadir']);
	
		//DESTINATION data directory should be relative
		$relative_datadir = cleanpath(spath($__settings['data_dir']));
...
function spath($path, $force = 0){
	
	global $__settings;
	
	if((empty($__settings['ftp_softpath']) || !empty($force)) && !empty($__settings['softpath'])){
		$__settings['ftp_softpath'] = str_replace($__settings['diff_path'], '', $__settings['softpath']);
	}
	...
	return str_replace($__settings['diff_path'], '', $path);
