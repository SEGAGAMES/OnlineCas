...
class dropbox{
	...
	function __curl($url, $headers = '', $filepointer = '', $upload_size = 0, $post = '', $download_file = '', $ignore_errors = 0, $retry = false){
		...
		if(empty($resp) && empty($retry) && empty($http_code)){
			$resp = $this->__curl($url, $headers, $filepointer, $upload_size, $post, $download_file, $ignore_errors, true);
		}
		
		$result = json_decode($resp, true);
		//r_print($result);
		
		//If files are locked in other operation
		if(strpos($result['error_summary'], 'too_many_write_operations') !== false){
			$resp = $this->__curl($url, $headers, $filepointer, $upload_size, $post, $download_file, $ignore_errors, true);
			$result = json_decode($resp, true);
		}
